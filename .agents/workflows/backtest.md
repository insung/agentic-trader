...
- **명령어**: `make backtest-fetch SYMBOL=종목 [TIMEFRAMES=H1] [FROM=YYYY-MM-DD] [TO=YYYY-MM-DD] [DAYS=일수]`
- **사용 예시**:
    - 최근 30일치 1시간봉: `make backtest-fetch SYMBOL=EURUSD DAYS=30`
    - 특정 기간 15분봉: `make backtest-fetch SYMBOL=BTCUSD FROM=2024-01-01 TO=2024-01-31 TIMEFRAMES=M15`
- **결과**: `backtests/data/` 디렉토리에 `{SYMBOL}_{TIMEFRAMES}_{START}_{END}.csv` 파일이 생성됩니다.
...
- **리포트 위치**: `backtests/reports/`
- **원본 결과 JSON**: `backtests/results/`
