...
이 파일은 Hermes Agent가 이 저장소에서 작업할 때 따르는 **공통 진입점이자 프로젝트 규칙 SSOT**입니다. 프로젝트 원칙은 이 파일과 `.agents/rules/project-rule.md`, `.agents/rules/document-rule.md`를 우선합니다.
...
- Hermes Agent의 내장 skill/tool을 우선 사용하고, 범용 CLI 호환 레이어를 늘리지 않습니다.
...
범용 CLI 호환 파일을 늘리는 방식으로 문제를 해결하지 말고, Python LangGraph workflow, FastAPI backend, deterministic guardrail, strategy validator, vertical slice 구조를 중심으로 코드를 작성하십시오.


## 9. Track workflow

- Track 기반 작업 관리가 필요하면 Hermes `track-agent` skill을 사용합니다.
- `track-agent`는 `~/.track/{project_name}/{YYYY-MM}-status.md`와 각 트랙의 `plan.md`, `audit.md`, `todos/`, `notes/`만 관리하는 얇은 트랙 레이어입니다.
- Planning은 Hermes `plan` / `writing-plans` skill을 우선 사용합니다.
- TDD는 `test-driven-development` skill과 이 저장소의 TDD Gate를 우선합니다.
- 설계 분기 승인은 Hermes `clarify` 도구로 처리합니다.
- `LESSONS.md`, `DECISIONS.md`는 기본 생성하지 않고, 장기 교훈은 Hermes memory 또는 프로젝트 공식 문서에 반영합니다.
- 사용자와 대화하며 계획·승인을 조율하는 상위 작업은 강한 모델을 쓰고, 하위 구현은 하나의 `todos/*.md`, plan 섹션, endpoint+tests, validator+tests처럼 응집된 작업 패키지 단위로 비용 효율적인 소형 모델에 위임할 수 있습니다.
