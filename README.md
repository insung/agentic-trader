...
자세한 테스트 절차는 [docs/testing-guide.md](docs/testing-guide.md), Paper/Live 실행 절차는 [docs/execution-guide.md](docs/execution-guide.md), 백테스트 절차는 [docs/backtesting-guide.md](docs/backtesting-guide.md)를 참고하십시오.
...
과거 OHLCV 데이터는 기본적으로 `backtests/data/market_data.sqlite`에 저장합니다. SQLite는 Docker 컨테이너가 아니라 Python이 직접 여는 로컬 파일입니다. `symbol`, `timeframe`, `time` 조합이 고유 키라 같은 기간을 다시 수집해도 캔들이 중복 저장되지 않습니다. 기존 CSV 입력은 임시 호환 경로로만 지원합니다. 저장소 구조는 [docs/sqlite-storage.md](docs/sqlite-storage.md)를 참고하십시오.

사용자가 직접 백테스트를 실행하고 결과를 확인하는 절차는 [docs/backtesting-guide.md](docs/backtesting-guide.md)에 정리되어 있습니다.
