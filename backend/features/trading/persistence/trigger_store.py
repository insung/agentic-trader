...
def set_schedule_rule_enabled(
    ...
    with _connect(db_path) as conn:
        conn.execute(
            "UPDATE trigger_schedule_rules SET enabled = ?, updated_at = datetime('now') WHERE rule_id = ?",
            (1 if enabled else 0, rule_id),
        )
        conn.commit()

def delete_schedule_rule(db_path: Optional[str] = None, rule_id: str = "") -> None:
    init_trigger_db(db_path)
    with _connect(db_path) as conn:
