...
현재 `trigger`는 1회 실행용입니다. 자동 매매 주기는 백엔드가 아닌 외부 스케줄러가 정합니다.
지금 구현된 서버 내 스케줄러는 `interval_seconds` 기반 규칙만 지원합니다. `cron` 기반 규칙은 계획 단계입니다.

### 예시: 15분마다 paper 트리거

```bash
while true; do
  cd /home/insung/Github/agentic-trader && make trigger SYMBOL=BTCUSD TIMEFRAMES=M15,M30 MODE=paper >> /home/insung/Github/agentic-trader/trading_logs/interval-paper.log 2>&1
  sleep 900
done
```
...
```bash
while true; do
  cd /home/insung/Github/agentic-trader && make trigger SYMBOL=BTCUSD TIMEFRAMES=M15,M30 MODE=live >> /home/insung/Github/agentic-trader/trading_logs/interval-live.log 2>&1
  sleep 900
done
```
...
- 서버 내 scheduler를 쓸 경우에는 `trigger_schedule_rules.interval_seconds`로 등록합니다.
- live 환경에서는 MT5가 켜져 있어야 하고, `make run-wine` 서버가 살아 있어야 합니다.
- 동일 심볼에 열린 포지션이 있으면 중복 진입 차단 정책을 먼저 점검합니다.
- 처음에는 `MODE=paper`로만 interval loop를 돌려 로그와 타이밍을 확인한 뒤, live로 전환합니다.
