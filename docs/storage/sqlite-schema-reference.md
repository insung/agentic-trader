...
trade_journals         trigger_id/trade_id 기준 운영 복기 라이프사이클
```
...
vectorbt 기반 빠른 전략 리서치 실행 1회의 메타데이터입니다. LangGraph/LLM 백테스트와 목적이 다르므로 `backtest_runs`와 분리합니다.

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | `INTEGER` | PK, AUTOINCREMENT | 내부 row ID입니다. |
| `run_id` | `TEXT` | NOT NULL, UNIQUE | quant research 실행 고유 ID입니다. |
| `strategy` | `TEXT` | NOT NULL | baseline 전략명입니다. 현재는 `bollinger`입니다. |
| `symbol` | `TEXT` | NOT NULL | 대상 심볼입니다. |
| `timeframe` | `TEXT` | NOT NULL | 단일 timeframe입니다. 예: `M15`. |
| `data_from` | `TEXT` | NOT NULL | 조회 시작일입니다. |
| `data_to` | `TEXT` | NOT NULL | 조회 종료일입니다. |
| `init_cash` | `REAL` | NOT NULL | 초기 자본입니다. |
| `fees` | `REAL` | NOT NULL, default `0.0` | vectorbt 수수료 입력값입니다. |
| `slippage` | `REAL` | NOT NULL, default `0.0` | vectorbt 슬리피지 입력값입니다. |
| `created_at` | `TEXT` | NOT NULL | 저장 시각입니다. |

### `quant_results`

`quant_runs`에 속한 파라미터 조합별 성과입니다.

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | `INTEGER` | PK, AUTOINCREMENT | 내부 row ID입니다. |
| `run_id` | `TEXT` | NOT NULL, FK | `quant_runs.run_id` 참조입니다. |
| `parameter_json` | `TEXT` | NOT NULL | Bollinger window/std, RSI, SL/TP 파라미터 JSON입니다. |
| `total_return_pct` | `REAL` | nullable | 총 수익률입니다. |
| `total_trades` | `INTEGER` | NOT NULL, default `0` | 총 거래 수입니다. |
| `win_rate` | `REAL` | nullable | 승률입니다. |
| `profit_factor` | `REAL` | nullable | Profit Factor입니다. |
| `max_drawdown_pct` | `REAL` | nullable | 최대 낙폭입니다. |
| `sharpe` | `REAL` | nullable | Sharpe Ratio입니다. |
| `expectancy` | `REAL` | nullable | vectorbt stats의 기대값입니다. |
| `rank` | `INTEGER` | NOT NULL | 기본 정렬 기준으로 부여한 순위입니다. |
| `created_at` | `TEXT` | NOT NULL | 저장 시각입니다. |
...
| `process_quality` | `TEXT` | nullable | 룰/과정 준수 품질입니다. |
| `outcome_quality` | `TEXT` | nullable | 결과/PnL 품질입니다. |
| `trade_quality_label` | `TEXT` | nullable | `good_trade`, `mixed_trade`, `bad_trade` 판정입니다. |
| `rule_adherence` | `INTEGER` | nullable | 전략/리스크 룰 준수 여부입니다. |
...
- 운영 DB는 2026-05-12 기준으로 `process_quality`, `outcome_quality`, `trade_quality_label`, `rule_adherence` 컬럼을 추가했고, 기존 `trade_reviews` row는 가능한 범위에서 백필했습니다.

### `trade_journals`

트리거 실행, 주문 체결, 청산, Risk Reviewer 복기를 한 row로 이어 붙인 운영 라이프사이클 테이블입니다. `trigger_id`로 trigger snapshot과 연결되고, `trade_id`로 open position 및 review 아카이브와 연결됩니다.

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `id` | `INTEGER` | PK, AUTOINCREMENT | 내부 journal row ID입니다. |
| `trade_id` | `TEXT` | NOT NULL, UNIQUE | 운영 복기 라이프사이클의 자연 키입니다. |
| `trigger_id` | `TEXT` | nullable | `trigger_runs.trigger_id`와 연결되는 트리거 ID입니다. |
| `workflow_run_id` | `TEXT` | nullable | LangGraph 워크플로우 실행 ID입니다. |
| `rule_id` | `TEXT` | nullable | 스케줄 규칙 ID입니다. |
| `mode` | `TEXT` | NOT NULL | `paper` 또는 `live` 입니다. |
| `symbol` | `TEXT` | NOT NULL | 거래 심볼입니다. |
| `action` | `TEXT` | NOT NULL | `BUY` 또는 `SELL` 입니다. |
| `status` | `TEXT` | NOT NULL | `open`, `closed`, `reviewed`, `review_failed` 중 하나입니다. |
| `opened_at` | `TEXT` | NOT NULL | 포지션이 열렸다고 기록된 시각입니다. |
| `closed_at` | `TEXT` | nullable | 포지션이 닫힌 시각입니다. |
| `reviewed_at` | `TEXT` | nullable | Risk Reviewer가 복기를 작성한 시각입니다. |
| `entry_price` | `REAL` | NOT NULL | 진입 가격입니다. |
| `exit_price` | `REAL` | nullable | 청산 가격입니다. |
| `sl` | `REAL` | NOT NULL | 손절 가격입니다. |
| `tp` | `REAL` | NOT NULL | 익절 가격입니다. |
| `lot_size` | `REAL` | NOT NULL | 주문 lot size입니다. |
| `result` | `TEXT` | nullable | 청산 결과 코드입니다. |
| `exit_reason` | `TEXT` | nullable | 청산 사유입니다. |
| `strategy` | `TEXT` | nullable | 선택된 전략명입니다. |
| `market_regime` | `TEXT` | nullable | 진입 당시 시장 상태입니다. |
| `review_id` | `TEXT` | nullable | 생성된 복기 Markdown / SQLite review ID입니다. |
| `review_markdown_path` | `TEXT` | nullable | 복기 Markdown 파일 경로입니다. |
| `decision_context_json` | `TEXT` | nullable | 진입 당시 판단 컨텍스트 JSON입니다. |
| `order_result_json` | `TEXT` | nullable | 주문 실행 결과 JSON입니다. |
| `closed_trade_json` | `TEXT` | nullable | 청산 시점 trade payload JSON입니다. |
| `review_log_json` | `TEXT` | nullable | Risk Reviewer structured output JSON입니다. |
| `created_at` | `TEXT` | NOT NULL | 최초 journal 저장 시각입니다. |
| `updated_at` | `TEXT` | NOT NULL | 마지막 journal 갱신 시각입니다. |

갱신 동작:

- 포지션 오픈 시 `status='open'`으로 저장됩니다.
- 청산 시 `status='closed'`로 갱신되고 `closed_trade_json`이 채워집니다.
- Risk Reviewer가 성공하면 `status='reviewed'`로 갱신되고 `review_id` / `review_log_json`이 채워집니다.
- 리뷰가 실패하면 `status='review_failed'`로 남아, 청산 정보는 보존되지만 복기 생성 실패를 추적할 수 있습니다.
...
| `trade_reviews.process_quality` | 과정/룰 준수 평가 | 익절/손절과 무관한 process quality |
| `trade_reviews.outcome_quality` | 결과/PnL 평가 | outcome quality 분리 |
| `trade_reviews.trade_quality_label` | 최종 trade 판정 | `good_trade` / `mixed_trade` / `bad_trade` |
| `trade_reviews.rule_adherence` | 룰 준수 여부 | 결과와 분리된 process gate |
| `trade_journals.decision_context_json` | 진입 당시 AI 판단 컨텍스트 | trigger -> trade -> review lifecycle 재구성 |
| `trade_journals.order_result_json` | MT5/Paper 주문 실행 결과 | 체결 후 처리 결과 추적 |
| `trade_journals.closed_trade_json` | 청산 시점 trade payload | 종료 사유, PnL, exit price 추적 |
| `trade_journals.review_log_json` | Risk Reviewer structured output 원문 | 복기 summary/risk/lesson 복원 |
...
sqlite3 backtests/data/market_data.sqlite \
  "SELECT 'candles', COUNT(*) FROM candles UNION ALL SELECT 'backtest_runs', COUNT(*) FROM backtest_runs UNION ALL SELECT 'backtest_trades', COUNT(*) FROM backtest_trades UNION ALL SELECT 'backtest_decisions', COUNT(*) FROM backtest_decisions UNION ALL SELECT 'backtest_reports', COUNT(*) FROM backtest_reports UNION ALL SELECT 'quant_runs', COUNT(*) FROM quant_runs UNION ALL SELECT 'quant_results', COUNT(*) FROM quant_results;"
```
...
최근 quant research 상위 결과:

```bash
sqlite3 -header -column backtests/data/market_data.sqlite \
  "SELECT qr.run_id, qr.symbol, qr.timeframe, q.rank, q.total_trades, ROUND(q.profit_factor, 2) AS pf, ROUND(q.max_drawdown_pct, 2) AS mdd, q.parameter_json FROM quant_runs qr JOIN quant_results q ON q.run_id = qr.run_id WHERE q.rank <= 5 ORDER BY qr.created_at DESC, q.rank ASC LIMIT 20;"
```
...
트리거별 trade lifecycle:

```bash
sqlite3 -header -column trading_logs/trading_logs.sqlite \
  "SELECT trigger_id, trade_id, symbol, status, result, exit_reason, reviewed_at FROM trade_journals WHERE trigger_id = 'trig_123' ORDER BY opened_at ASC;"
```
